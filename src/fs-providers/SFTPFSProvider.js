...
class SFTPFSProvider extends RemoteFileSystemProvider_1.default {
    connect(remote) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const shouldPromptForPass = password == undefined && remote.agent == undefined && remote.privateKeyPath == undefined;
            // tslint:enable
            ...
            if (passphrase === true) {
                // modify remote so we don't need later
                passphrase = yield host_1.promptForPassword('Enter your passphrase');
            }
            ...
            return this._connectClient(Object.assign({}, connectOption, { password,
                passphrase }));
        });
    }
            ...
            client.lstat(uri.path, (err, stat) => {
                ...
                if (filetype === vscode.FileType.SymbolicLink) {
                    return this._realFileType(uri, client)
                        .then(realType => ({
                        type: vscode.FileType.SymbolicLink | realType,
                        ctime: 0,
        ...
        return new Promise((resolve, reject) => {
            client.readdir(uri.path, (err, stats) => {
                ...
                const promises = stats.map(stat => {
                    ...
                    if (fileType === vscode.FileType.SymbolicLink) {
                        return this._realFileType(uri.with({ path: upath.join(uri.path, filename) }), client).then(realType => [
                            filename,
                            vscode.FileType.SymbolicLink | realType,
                        ]);
                    }
